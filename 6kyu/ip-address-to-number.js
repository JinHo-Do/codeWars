...
function ipToNum(ip) {
  let splitted = ip.split('.').map(v => {
    let checkLen = parseInt(v, 10).toString(2);
    return checkLen = '0'.repeat(8 - checkLen.length) + checkLen;
  });
...
function numToIp(num) {
  let convertBin = num.toString(2).split('').reverse().join('');
  let splitted = ''

  for (let i = 0; i < convertBin.length; i++) {
    if (i !== 0 && i % 8 === 0) {
      splitted += ',' + convertBin[i];
    } else {
      splitted += convertBin[i];
    }
  }

  splitted = splitted.split('').reverse().join('');

  return splitted.split(',').map(v => parseInt(v, 2)).join('.');
};
