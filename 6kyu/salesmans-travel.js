...
function travel(r, zipcode) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let sameIdx = arr[i].indexOf(zipcode);
    let checkCode = arr[i].slice(sameIdx);

    if (sameIdx > -1 && checkCode === zipcode) {
      var tempStr = '';
      var tempArr = arr[i].split(' ');
      var tempNum = tempArr.splice(0, 1);
  ...
  return !outArr ? zip + '/' :zip + outArr.join(',') + '/' + outNum.join(',');
}
