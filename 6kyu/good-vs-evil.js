...
function goodVsEvil(good, evil){
  let goodSoldier = [1, 2, 3, 3, 4, 10], evilSoldier = [1, 2, 2, 2, 3, 5, 10];
  let goodPower = good.split(' ').map((v, i) => +v * goodSoldier[i]).reduce((a, b) => a + b);
  let evilPower = evil.split(' ').map((v, i) => +v * evilSoldier[i]).reduce((a, b) => a + b);
