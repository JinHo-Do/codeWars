...
var Vector = function (components) {
  components.add = function(arr) {
    if (this.length !== arr.length) throw err;
    return this.map((v, i) => v + arr[i]);
  };

  components.subtract = function(arr) {
    if (this.length !== arr.length) throw err;
    return this.map((v, i) => v - arr[i]);
  };

  components.dot = function(arr) {
    if (this.length !== arr.length) throw err;
    return this.map((v, i) => v * arr[i]).reduce((a, b) => a + b);
  };

  components.norm = function() {
    return Math.sqrt(this.map((v, i) => Math.pow(v, 2)).reduce((a, b) => a + b));
  };
